...
//Get a list of all members who have subscribed to the CiviCRM mailing list (via Mailchimp) since last time script ran (assuming it runs daily).
...
class mailchimpSync {
	...
	function __construct($debugging = 'off', $display_errors = 0) {
		...
		$this->apikey = (isset($_GET['apikey'])) ? $_GET['apikey'] : "000000000000000000000000000000000";
	...
	public function createTables() {
		...
		$sql = "SELECT table_name FROM information_schema.tables 
				WHERE table_schema IN ('civicrm_mailchimp_sync_log', 'civicrm_mailchimp_sync_log_removes', 'civicrm_mailchimp_sync_log_groups', 'civicrm_mailchimp_sync_log_contacts') 
				AND table_name IN ('civicrm_mailchimp_sync_log', 'civicrm_mailchimp_sync_log_removes', 'civicrm_mailchimp_sync_log_groups', 'civicrm_mailchimp_sync_log_contacts')";

		$result = mysql_query($sql);
		
		if (!$result) {
			echo "Fatal Error: Unable to create tables for proper functioning of this script.\n\n";
			print_r(mysql_error());
			die();
		}
	...
	public function addGroups() {
		...
		$sql = "SELECT id, name, title FROM civicrm_group WHERE (group_type = '2' OR group_type = '12') AND is_active = '1'";
		
		echo "\n\nAdding CiviCRM groups (of type mailing list) to Mailchimp\n\n";
	...
	public function addContacts() {
		...
		if ($this->debugging == 'on') { echo "Adding contacts that are in CiviCRM Groups to MailChimp (that have been added from CiviCRM groups)...\n\n"; }
		...
		while ($contact = mysql_fetch_array($group_list)) {
			...
			$return_result = $this->api->listSubscribe($this->list_id, $contact['email'], $buffer, 'html', false, true, true, false);
			
			if ($return_result) {
				if ($this->debugging == 'on') echo "Successfully subscribed ".$contact['email']."\n\n";
			}
			else {
				echo "Unable to subscribe ".$contact['email']."\n\n";
				if ($this->api->errorCode){
	...
	public function unsubscribeOnHold() {
		...
		if ($this->debugging == 'on') echo "Unsubscribing contacts from Mailchimp that have the 'On Hold' setting set in CiviCRM";
	...
	public function addContactsSubscribed() {
		...
		if (isset($new_member_data['data']) && !empty($new_member_data['data'])) {
			foreach ($new_member_data['data'] as $new_member) {
				...
				if ($this->debugging == 'on') echo "FIRST: DETERMINE IF CONTACT ALREADY EXISTS IN CIVICRM\n\n";
